# utils/kpi_center_performance/payment/payment_analysis.py
"""
Payment & Collection Analysis for KPI Center Performance.

Adapted from salesperson_performance/payment/payment_analysis.py.

DATA SOURCE:
  customer_ar_by_kpi_center_view:
    - Pre-calculated columns: outstanding_by_kpi_center_usd, collected_by_kpi_center_usd,
      days_overdue, aging_bucket, gp_outstanding_by_kpi_center_usd
    - Split by KPI Center (split_rate_percent)
...
# =============================================================================
# COLUMN CONFIGURATION — KPI Center columns
# =============================================================================

REV_COL = 'sales_by_kpi_center_usd'
GP_COL = 'gross_profit_by_kpi_center_usd'
GP1_COL = 'gp1_by_kpi_center_usd'

# Pre-calculated columns from customer_ar_by_kpi_center_view
PRECALC_OUTSTANDING_COL = 'outstanding_by_kpi_center_usd'
PRECALC_COLLECTED_COL = 'collected_by_kpi_center_usd'
PRECALC_GP_OUTSTANDING_COL = 'gp_outstanding_by_kpi_center_usd'
PRECALC_DAYS_OVERDUE_COL = 'days_overdue'
PRECALC_AGING_BUCKET_COL = 'aging_bucket'

# Actual line-level amounts (not split-allocated)
...
# Alert thresholds
OVERDUE_90_THRESHOLD_USD = 10000
COLLECTION_RATE_LOW = 0.70
LARGE_UNPAID_THRESHOLD_USD = 50000
OVERDUE_SHARE_THRESHOLD = 0.15
...
def analyze_payments(
    ...
    """
    Analyze payment & collection from KPI Center data.

    Args:
        sales_df: Filtered DataFrame with payment columns
        as_of_date: Reference date for aging (default: today)

    Returns:
        Dict with payment metrics, or None if no payment data.
    """
    if sales_df.empty:
        return None
    ...
    # Pre-calculated vs fallback
    has_precalc = PRECALC_OUTSTANDING_COL in df.columns

    if has_precalc:
        df['_collected_usd'] = pd.to_numeric(df[PRECALC_COLLECTED_COL], errors='coerce').fillna(0)
        df['_outstanding_usd'] = pd.to_numeric(df[PRECALC_OUTSTANDING_COL], errors='coerce').fillna(0)
        if PRECALC_GP_OUTSTANDING_COL in df.columns:
            df['_outstanding_gp'] = pd.to_numeric(df[PRECALC_GP_OUTSTANDING_COL], errors='coerce').fillna(0)
        elif GP_COL in df.columns:
            df['_outstanding_gp'] = df[GP_COL] * (1 - df['payment_ratio'])
    else:
        logger.warning("Pre-calculated columns not found — falling back to proxy.")
    ...
    # 4. BY KPI CENTER
    by_kpi_center = _analyze_by_kpi_center(df)
...
def _calculate_aging(outstanding_df: pd.DataFrame, today: date) -> pd.DataFrame:
    """Calculate aging buckets. Uses pre-calculated aging_bucket if available."""
    ...
    # FAST PATH: pre-calculated aging_bucket from AR view
    if PRECALC_AGING_BUCKET_COL in df.columns and df[PRECALC_AGING_BUCKET_COL].notna().any():
        ...
        result['share'] = np.where(total_outstanding > 0, result['amount'] / total_outstanding, 0)
        ...
        result['_order'] = result['bucket'].map({k: i for i, k in enumerate(bucket_order.keys())}).fillna(99)
        result['min_days'] = result['bucket'].map(lambda b: bucket_order.get(b, (0, 0))[0])
        result['max_days'] = result['bucket'].map(lambda b: bucket_order.get(b, (0, 0))[1])
    ...
    for bucket_name, min_d, max_d in buckets:
        ...
        rows.append({
            'bucket': bucket_name, 'min_days': min_d, 'max_days': max_d,
            'amount': amount, 'gp': gp, 'count': count, 'share': share,
        })
...
def _analyze_by_customer(outstanding_df: pd.DataFrame, today: date) -> pd.DataFrame:
    ...
    elif 'inv_date' in df.columns:
        df['_days'] = (today_ts - df['inv_date']).dt.days
    else:
        df['_days'] = 0
...
def _analyze_by_kpi_center(df: pd.DataFrame) -> pd.DataFrame:
    """Collection rate by KPI Center."""
    if df.empty or 'kpi_center' not in df.columns:
        return pd.DataFrame()

    result = df.groupby(['kpi_center_id', 'kpi_center']).agg(
...
def _analyze_by_month(df: pd.DataFrame) -> pd.DataFrame:
    ...
    result['collection_rate'] = np.where(
        result['invoiced'] > 0, result['collected'] / result['invoiced'], 0
    )
...
def render_payment_section(payment_data: Optional[Dict]):
    """Render payment/collection analysis section (Overview & Aging tab)."""
    if not payment_data:
...
def _build_aging_chart(aging_df: pd.DataFrame) -> alt.Chart:
    ...
    bars = alt.Chart(df).mark_bar().encode(
        ...
        color=alt.Color('bucket:N', scale=alt.Scale(domain=domain, range=range_colors), legend=None),
        tooltip=[
    ...
    text_labels = alt.Chart(df).mark_text(align='left', dx=4, fontSize=11).encode(
        y=alt.Y('bucket:N', sort=alt.EncodingSortField(field='bucket_order')),
        x=alt.X('amount:Q'),
        text=alt.Text('amount:Q', format='$,.2f'),
    )

    return (bars + text_labels).properties(height=35 * max(len(df), 2) + 40, title='Outstanding by Aging')
...
def _build_collection_trend_chart(by_month: pd.DataFrame) -> alt.Chart:
    ...
    bars = alt.Chart(melted).mark_bar().encode(
        ...
        color=alt.Color('type:N', scale=alt.Scale(
            domain=['Invoiced', 'Collected'], range=['#aec7e8', '#28a745']
    ...
    line = alt.Chart(df).mark_line(color='#800080', strokeWidth=2, point=True).encode(
        x=alt.X('inv_month:N'),
        y=alt.Y('collection_rate:Q', title='Collection Rate', axis=alt.Axis(format='.1%')),
        tooltip=[
            alt.Tooltip('inv_month:N', title='Month'),
    ...
    return alt.layer(bars, line).resolve_scale(y='independent').properties(height=280, title='Monthly Collection Trend')